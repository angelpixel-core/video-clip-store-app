...
IMAGE_NAME="${IMAGE_NAME:-video-clip-store-app:test}"
TEST_DB_CONTAINER_NAME="${TEST_DB_CONTAINER_NAME:-video-clip-store-app-test-db}"
...
ensure_database() {
  ...
  docker network inspect video_clip_store_app_net >/dev/null 2>&1 ||
    docker network create --driver bridge video_clip_store_app_net >/dev/null
  docker rm -f video-clip-store-app-db >/dev/null 2>&1 || true
  docker rm -f "$TEST_DB_CONTAINER_NAME" >/dev/null 2>&1 || true
  docker run -d \
    --name "$TEST_DB_CONTAINER_NAME" \
    --network video_clip_store_app_net \
    --network-alias db \
